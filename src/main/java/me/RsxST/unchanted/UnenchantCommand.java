...
public record UnenchantCommand(Unchanted plugin) implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender,
        ...
        if (!player.hasPermission("unenchant.use")) {
            player.sendMessage(plugin.format("&cYou do not have permission to use this command"));
            return true;
        }
        ...
        if (args.length == 2 && args[0].equalsIgnoreCase("remove")) {
            ...
            // Modern Paper registry lookup instead of deprecated getByKey
            ...
            if (enchant == null) {
                player.sendMessage(plugin.format("&cUnknown enchantment: &e" + enchantName));
                return true;
            }

            var item = player.getInventory().getItemInMainHand();
            if (!item.containsEnchantment(enchant)) {
                player.sendMessage(plugin.format("&cYour item does not have that enchantment!"));
                return true;
            }

            int level = item.getEnchantmentLevel(enchant);
            plugin.giveBook(player, enchant, level);
            item.removeEnchantment(enchant);
            player.sendMessage(plugin.format("&aRemoved &e" + enchant.getKey().getKey() + " &afrom your item!"));
            return true;
        }

        // Fallback message for wrong usage
        player.sendMessage(plugin.format("&cUsage: /unenchant [all] or click an enchantment to remove it"));
        return true;
    }
}
